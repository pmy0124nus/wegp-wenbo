import numpy as np
import scipy.linalg
import scipy.optimize
import random
import torch
from scipy.stats import norm
from wegp_bayes.optim.acquisition_functions import EI
import time
from typing import Callable, List, Sequence, Tuple, Optional

"""
改进版离散GP优化策略
- 结合局部搜索和全局搜索来找到更好的起始点
- 提供多种优化策略选择
- 保持与原始代码的兼容性

主要改进：
1. 局部搜索改进起始点
2. 多种全局搜索策略
3. 混合优化策略
4. 可配置的策略选择
"""

# ============================================================
# 工具函数
# ============================================================
def ensure_finite(name: str, arr: np.ndarray):
    """检查数组是否全为有限值"""
    if not np.all(np.isfinite(arr)):
        idx = np.where(~np.isfinite(arr))[0]
        raise ValueError(f"{name} 含有 NaN/Inf，位置 {idx}，值 {arr[idx]}")


def add_jitter(K: np.ndarray, jitter: float = 1e-8) -> np.ndarray:
    """给核矩阵加抖动，确保可 Cholesky 分解"""
    return K + jitter * np.eye(K.shape[0], dtype=K.dtype)



# ============================================================
# Discrete GP (与原版相同)
# ============================================================
class DiscreteGP:
    def __init__(self, index_number: int, kernel: np.ndarray):
        """
        index_number: 离散元素个数
        kernel: 形状 [n, n] 的 SPD 核矩阵
        """
        self.n = index_number
        self.K = np.asarray(kernel, dtype=float)
        if self.K.shape != (self.n, self.n):
            raise ValueError(f"kernel 维度应为 ({self.n},{self.n}), 但得到 {self.K.shape}")
        self.observed_idx: List[int] = []
        self.A_obs: List[float] = []

    def update(self, idx_list: Sequence[int], A_list: Sequence[float]):
        idx_list = list(map(int, idx_list))
        A_arr = np.asarray(A_list, dtype=float).reshape(-1)
        if len(idx_list) != len(A_arr):
            raise ValueError("update: idx_list 与 A_list 长度不一致")
        # 过滤 NaN/Inf
        finite_mask = np.isfinite(A_arr)
        if not np.all(finite_mask):
            print("Warning: 丟弃非有限 A 值:", A_arr[~finite_mask])
        idx_list = np.array(idx_list)[finite_mask].tolist()
        A_arr = A_arr[finite_mask]

        # 允许重复索引：策略是直接追加。也可以选择平均化或只保留最新一次
        self.observed_idx.extend(idx_list)
        self.A_obs.extend(A_arr.tolist())

    def predict(self) -> Tuple[np.ndarray, np.ndarray]:
        if not self.observed_idx:
            mu = np.zeros(self.n, dtype=float)
            var = np.diag(self.K).copy()
            return mu, var

        o = self.observed_idx
        A = np.asarray(self.A_obs, dtype=float)
        assert len(o) == len(A), "observed_idx 与 A_obs 长度不相等"

        Koo = self.K[np.ix_(o, o)].astype(float)
        Kof = self.K[np.ix_(o, range(self.n))].astype(float)

        # 检查有限性
        ensure_finite("A_obs", A)
        ensure_finite("Koo", Koo)
        ensure_finite("Kof", Kof)

        # jitter
        Koo_j = add_jitter(Koo, jitter=1e-8)

        # Cholesky
        L = scipy.linalg.cholesky(Koo_j, lower=True, check_finite=True)
        alpha = scipy.linalg.cho_solve((L, True), A, check_finite=True)

        mu = Kof.T @ alpha
        v = scipy.linalg.cho_solve((L, True), Kof, check_finite=True)
        var = np.diag(self.K) - np.sum(Kof * v, axis=0)
        var = np.maximum(var, 0.0)  # 防止微小负数

        return mu, var


# ============================================================
# 改进版 Mixed Bayesian Optimization with Discrete GP
# ============================================================
class MixedBayesOptGPDiscreteImproved:
    def __init__(
        self,
        acq_obj,
        rng: np.random.RandomState,
        config_fun,
        combined_cat_index: np.ndarray,
        model,
        kernel: Optional[np.ndarray] = None,
    ):
        """
        rng: numpy RandomState
        config_fun: 连续空间采样工具
        combined_cat_index: shape [n_cat, m] 的离散特征编码矩阵（每个离散元素一行）
        model: surrogate，用于 EI
        kernel: 离散核矩阵
        """

        self.rng = rng
        self.config_fun = config_fun
        self.combined_cat_index = np.asarray(combined_cat_index, dtype=float)
        self.n = self.combined_cat_index.shape[0]
        self.model = model
        self.acq_obj = acq_obj
        self.dgps = DiscreteGP(self.n, kernel)

        # 添加计数器
        self.evaluate_count = 0

        # 为了演示，也记录每一次评估
        self.history = []

    def _encode_input(self, h_idx: int, x_cont: np.ndarray) -> torch.Tensor:
        """
        将离散选择的 one-hot / embedding 与连续变量拼接成一个 torch tensor
        """
        h_vec = self.combined_cat_index[h_idx]  # shape [m]
        arr = np.concatenate([x_cont, h_vec]).astype(np.float32)
        return torch.from_numpy(arr)

    def _evaluate_batch(self, h_idx: int, x_candidates: List[np.ndarray]) -> np.ndarray:
        """
        批量评估多个候选点，大幅提升效率
        - h_idx: 离散选项索引
        - x_candidates: 候选点列表
        返回：EI值数组
        """
        if not x_candidates:
            return np.array([])
        
        # 批量编码
        encoded_inputs = []
        for x in x_candidates:
            z_t = self._encode_input(h_idx, x)
            encoded_inputs.append(z_t)
        
        # 批量评估（如果acq_obj支持批量评估）
        try:
            # 尝试批量评估
            batch_tensor = torch.stack(encoded_inputs)
            batch_ei = self.acq_obj.evaluate_batch(batch_tensor, num_samples=100)
            self.evaluate_count += len(x_candidates)
            return batch_ei
        except (AttributeError, NotImplementedError):
            # 如果不支持批量评估，回退到逐个评估
            ei_values = []
            for z_t in encoded_inputs:
                val = self._evaluate_with_counting(z_t)
                ei_values.append(val)
            return np.array(ei_values)

    def _evaluate_with_counting(self, x_tensor):
        """
        包装evaluate方法，同时计数
        """
        self.evaluate_count += 1
        return self.acq_obj.evaluate(x_tensor, num_samples=100)

    def _local_search_continuous(self, h_idx: int, x0: np.ndarray, max_iter: int = 20, sigma: float = 0.1, delta: float = 0.3) -> Tuple[np.ndarray, float]:
        """
        对连续变量进行局部搜索，改进起始点
        - h_idx: 离散选项索引
        - x0: 初始连续变量
        - max_iter: 最大迭代次数
        - sigma: 高斯采样标准差
        - delta: 距离阈值
        返回： (best_x, best_ei)
        """
        # 确保x0是numpy数组
        x_t = np.asarray(x0).copy()
        z_t = self._encode_input(h_idx, x_t)
        best_ei = self._evaluate_with_counting(z_t)
        best_x = x_t.copy()
        
        no_improve = 0
        max_no_improve = 5
        
        for iter in range(max_iter):
            # 在x_t周围采样候选点
            candidates = []
            for _ in range(10):  # 每次采样10个候选
                cand = x_t + np.random.randn(*x_t.shape) * sigma
                # 确保在边界内
                cand = np.clip(cand, 0.0, 1.0)
                # 检查距离
                if np.linalg.norm(cand - x_t) <= delta:
                    candidates.append(cand)
            
            if not candidates:
                continue
                
            # 批量评估所有候选点
            if candidates:
                cand_eis = self._evaluate_batch(h_idx, candidates)
                best_idx = np.argmax(cand_eis)
                best_cand_ei = cand_eis[best_idx]
                best_cand = candidates[best_idx]
            else:
                best_cand_ei = -np.inf
                best_cand = None
            
            # 如果找到更好的点，更新
            if best_cand_ei > best_ei + 1e-6:
                x_t = best_cand
                best_ei = best_cand_ei
                best_x = best_cand.copy()
                no_improve = 0
            else:
                no_improve += 1
            
            # 如果连续多次没有改进，提前停止
            if no_improve >= max_no_improve:
                break
        
        return best_x, best_ei

    def _global_search_continuous(self, h_idx: int, n_samples: int = 20) -> List[np.ndarray]:
        """
        全局搜索策略：使用多种采样方法
        - h_idx: 离散选项索引
        - n_samples: 采样数量
        返回：候选点列表
        """
        candidates = []
        n_quant = len(self.config_fun.quant_index)
        
        # 1. 拉丁超立方采样
        lhd_samples = int(n_samples * 0.4)
        if lhd_samples > 0:
            rng = np.random.RandomState(42 + h_idx)
            lhd_z = self.config_fun.latinhypercube_sample(rng, lhd_samples)
            lhd_x = lhd_z[..., self.config_fun.quant_index]
            candidates.extend(lhd_x)
        
        # 2. 随机采样
        random_samples = int(n_samples * 0.3)
        if random_samples > 0:
            for _ in range(random_samples):
                x = np.random.uniform(0.0, 1.0, n_quant)
                candidates.append(x)
        
        # 3. 边界采样
        boundary_samples = int(n_samples * 0.2)
        if boundary_samples > 0:
            for _ in range(boundary_samples):
                x = np.random.choice([0.0, 1.0], size=n_quant)
                # 添加一些噪声
                x += np.random.normal(0, 0.1, n_quant)
                x = np.clip(x, 0.0, 1.0)
                candidates.append(x)
        
        # 4. 中心采样
        center_samples = n_samples - len(candidates)
        if center_samples > 0:
            for _ in range(center_samples):
                x = np.random.normal(0.5, 0.2, n_quant)
                x = np.clip(x, 0.0, 1.0)
                candidates.append(x)
        
        return candidates

    def optimize_x_given_h_hybrid(self, h_idx: int, n_starts: int = 200, top_k: int = 3, 
                                  use_local_search: bool = True, use_global_search: bool = True,
                                  use_robust_optimizer: bool = True) -> Tuple[np.ndarray, float]:
        """
        混合优化策略：结合局部搜索和全局搜索来找到更好的起始点
        - h_idx: 离散选项索引
        - n_starts: 采样多少个初始点
        - top_k: 选出多少个最优点作为L-BFGS起点
        - use_local_search: 是否使用局部搜索
        - use_global_search: 是否使用全局搜索
        - use_robust_optimizer: 是否使用更稳健的优化器设置
        返回： (x_opt, A_val) 其中 A_val = EI(x_opt)
        """
        all_starters = []
        all_eis = []
        
        # 1. 随机采样起始点
        seed = 12345 + h_idx * 1000
        rng = np.random.RandomState(seed)
        init_z = self.config_fun.latinhypercube_sample(rng, n_starts)
        x_candidates = init_z[..., self.config_fun.quant_index]
        
        # 批量评估随机起始点
        all_eis = self._evaluate_batch(h_idx, x_candidates.tolist())
        all_starters = x_candidates.tolist()
        
        # 2. 局部搜索改进（如果启用）
        if use_local_search:
            # 选择最好的几个随机点进行局部搜索
            best_random_idx = np.argsort(all_eis)[-min(5, len(all_eis)):]
            improved_xs = []
            improved_eis = []
            for idx in best_random_idx:
                # 确保idx是整数
                idx = int(idx)
                improved_x, improved_ei = self._local_search_continuous(h_idx, all_starters[idx])
                improved_xs.append(improved_x)
                improved_eis.append(improved_ei)
            
            # 将改进的结果添加到列表中
            all_starters.extend(improved_xs)
            all_eis = np.concatenate([all_eis, np.array(improved_eis)])
        
        # 3. 全局搜索（如果启用）
        if use_global_search:
            # 使用更激进的采样策略
            global_candidates = self._global_search_continuous(h_idx, n_samples=20)
            if global_candidates:
                global_eis = self._evaluate_batch(h_idx, global_candidates)
                all_starters.extend(global_candidates)
                all_eis = np.concatenate([all_eis, global_eis])
        
        # 4. 选择最好的top_k个点进行优化
        all_eis = np.array(all_eis)
        best_indices = np.argsort(all_eis)[-top_k:]
        starters = np.array([all_starters[int(i)] for i in best_indices])
        
        # 5. 优化策略选择
        best_x = None
        best_ei = -np.inf
        
        for x0 in starters:
            def obj(x_numpy: np.ndarray) -> float:
                z_t = self._encode_input(h_idx, x_numpy)
                val = self._evaluate_with_counting(z_t)
                return -val

            n_quant = len(self.config_fun.quant_index)
            bounds = [(0.0, 1.0)] * n_quant
            
            if use_robust_optimizer:
                # 使用更稳健的优化设置
                try:
                    # 尝试L-BFGS-B with 更严格的收敛条件
                    res = scipy.optimize.minimize(
                        obj, x0, method="L-BFGS-B", bounds=bounds,
                        options={
                            'maxiter': 100,
                            'gtol': 1e-6,
                            'ftol': 1e-8,
                            'eps': 1e-8
                        }
                    )
                    
                    # 如果L-BFGS失败，尝试其他方法
                    if not res.success:
                        # 尝试Powell方法（不需要梯度）
                        res_powell = scipy.optimize.minimize(
                            obj, x0, method="Powell", bounds=bounds,
                            options={'maxiter': 200}
                        )
                        
                        # 选择更好的结果
                        if res_powell.fun < res.fun:
                            res = res_powell
                            
                except Exception as e:
                    print(f"优化失败，使用原始点: {e}")
                    A_val = -obj(x0)
                    if np.isfinite(A_val) and A_val > best_ei:
                        best_ei = A_val
                        best_x = x0
                    continue
            else:
                # 原始L-BFGS设置
                res = scipy.optimize.minimize(obj, x0, method="L-BFGS-B", bounds=bounds)

            A_val = -res.fun
            if np.isfinite(A_val) and A_val > best_ei:
                best_ei = A_val
                best_x = res.x

        # fallback
        if best_x is None:
            best_x = starters[0]
            best_ei = all_eis[best_indices[0]]

        return best_x, best_ei

    def optimize_x_given_h_original(self, h_idx: int, n_starts: int = 100, top_k: int = 10) -> Tuple[np.ndarray, float]:
        """
        原始优化策略（与原版相同）
        """
        # 用固定种子+ h_idx 采样 starter
        seed = 12345 + h_idx * 1000
        rng = np.random.RandomState(seed)
        init_z = self.config_fun.latinhypercube_sample(rng, n_starts)
        x_candidates = init_z[..., self.config_fun.quant_index]

        # 批量计算每个点的EI
        ei_list = self._evaluate_batch(h_idx, x_candidates.tolist())

        # 选出 top_k 个EI最大的点作为起点
        if len(ei_list) < top_k:
            top_k = len(ei_list)
        topk_idx = np.argsort(ei_list)[-top_k:]
        starters = x_candidates[topk_idx]

        # 对每个starter做L-BFGS优化
        best_x = None
        best_ei = -np.inf
        for x0 in starters:
            def obj(x_numpy: np.ndarray) -> float:
                z_t = self._encode_input(h_idx, x_numpy)
                val = self._evaluate_with_counting(z_t)
                return -val

            n_quant = len(self.config_fun.quant_index)
            bounds = [(0.0, 1.0)] * n_quant
            res = scipy.optimize.minimize(obj, x0, method="L-BFGS-B", bounds=bounds)

            A_val = -res.fun
            if np.isfinite(A_val) and A_val > best_ei:
                best_ei = A_val
                best_x = res.x

        # fallback
        if best_x is None:
            best_x = starters[0]
            best_ei = ei_list[topk_idx[0]]

        return best_x, best_ei

    def optimize_x_given_h_complex_robust(self, h_idx: int, n_starts: int = 300, 
                                         use_multiple_methods: bool = True) -> Tuple[np.ndarray, float]:
        """
        专门处理复杂函数的鲁棒优化策略
        - 使用多种优化方法
        - 更好的数值稳定性
        - 针对噪声函数的处理
        """
        # 1. 采样更多起始点
        seed = 12345 + h_idx * 1000
        rng = np.random.RandomState(seed)
        init_z = self.config_fun.latinhypercube_sample(rng, n_starts)
        x_candidates = init_z[..., self.config_fun.quant_index]
        
        # 2. 批量评估并选择最好的候选点
        all_eis = self._evaluate_batch(h_idx, x_candidates.tolist())
        top_k = min(10, len(all_eis))
        best_indices = np.argsort(all_eis)[-top_k:]
        starters = x_candidates[best_indices]
        
        best_x = None
        best_ei = -np.inf
        n_quant = len(self.config_fun.quant_index)
        bounds = [(0.0, 1.0)] * n_quant
        
        def obj(x_numpy: np.ndarray) -> float:
            z_t = self._encode_input(h_idx, x_numpy)
            val = self._evaluate_with_counting(z_t)
            return -val
        
        # 3. 使用多种优化方法
        optimization_methods = []
        
        if use_multiple_methods:
            # L-BFGS-B with 不同参数
            optimization_methods.extend([
                ("L-BFGS-B", {"maxiter": 50, "gtol": 1e-4, "ftol": 1e-6}),
                ("L-BFGS-B", {"maxiter": 100, "gtol": 1e-6, "ftol": 1e-8}),
                ("L-BFGS-B", {"maxiter": 200, "gtol": 1e-8, "ftol": 1e-10}),
            ])
            
            # 无梯度方法
            optimization_methods.extend([
                ("Powell", {"maxiter": 100}),
                ("Powell", {"maxiter": 200}),
                ("Nelder-Mead", {"maxiter": 200}),
            ])
        else:
            # 只使用L-BFGS-B
            optimization_methods.append(("L-BFGS-B", {"maxiter": 100, "gtol": 1e-6, "ftol": 1e-8}))
        
        # 4. 对每个起始点尝试多种方法
        for x0 in starters:
            for method_name, options in optimization_methods:
                try:
                    if method_name == "L-BFGS-B":
                        res = scipy.optimize.minimize(obj, x0, method="L-BFGS-B", bounds=bounds, options=options)
                    else:
                        # 无梯度方法不支持bounds，需要手动处理
                        res = scipy.optimize.minimize(obj, x0, method=method_name, options=options)
                        # 确保结果在边界内
                        res.x = np.clip(res.x, 0.0, 1.0)
                    
                    A_val = -res.fun
                    if np.isfinite(A_val) and A_val > best_ei:
                        best_ei = A_val
                        best_x = res.x.copy()
                        
                except Exception as e:
                    # 如果优化失败，继续下一个方法
                    continue
        
        # 5. 如果所有优化都失败，使用最好的起始点
        if best_x is None:
            best_x = starters[0]
            best_ei = all_eis[best_indices[0]]
        
        return best_x, best_ei

    def run(self, init_size: int, N_cand: int, T: int, strategy: str = "hybrid"):
        """
        init_size: 初始随机选择的离散点数
        N_cand: 每轮选取的离散候选个数
        T: 总迭代次数
        strategy: 优化策略 ("original", "hybrid", "local_only", "global_only")
        """

        # 1. 初始采样
        init_idxs = self.rng.choice(self.n, size=init_size, replace=False).tolist()
        for idx in init_idxs:
            if strategy == "original":
                x_opt, A_val = self.optimize_x_given_h_original(idx)
            elif strategy == "hybrid":
                x_opt, A_val = self.optimize_x_given_h_hybrid(idx)
            elif strategy == "local_only":
                x_opt, A_val = self.optimize_x_given_h_hybrid(idx, use_global_search=False)
            elif strategy == "global_only":
                x_opt, A_val = self.optimize_x_given_h_hybrid(idx, use_local_search=False)
            else:
                raise ValueError(f"未知策略: {strategy}")
                
            print(f"init_idxs: {idx}, x_opt: {x_opt}, A_val: {A_val}")
            self.dgps.update([idx], [A_val])

        # 2. 迭代
        for t in range(init_size, T):
            mu, var = self.dgps.predict()
            # 取均值最大的 N_cand 作为候选
            cand_idxs = np.argsort(mu)[-N_cand:]

            refined = []
            for idx in cand_idxs:
                if strategy == "original":
                    x_opt, A_val = self.optimize_x_given_h_original(idx)
                elif strategy == "hybrid":
                    x_opt, A_val = self.optimize_x_given_h_hybrid(idx)
                elif strategy == "local_only":
                    x_opt, A_val = self.optimize_x_given_h_hybrid(idx, use_global_search=False)
                elif strategy == "global_only":
                    x_opt, A_val = self.optimize_x_given_h_hybrid(idx, use_local_search=False)
                else:
                    raise ValueError(f"未知策略: {strategy}")
                    
                refined.append((idx, x_opt, A_val))
                self.dgps.update([idx], [A_val])

        # 3. 最终选择（基于 GP 均值）
        mu, var = self.dgps.predict()
        best_idx = int(np.argmax(mu))
        print(f"mu: {mu}")
        print(f"best_idx: {best_idx}")
        
        if strategy == "original":
            best_x, best_acq = self.optimize_x_given_h_original(best_idx)
        elif strategy == "hybrid":
            best_x, best_acq = self.optimize_x_given_h_hybrid(best_idx)
        elif strategy == "local_only":
            best_x, best_acq = self.optimize_x_given_h_hybrid(best_idx, use_global_search=False)
        elif strategy == "global_only":
            best_x, best_acq = self.optimize_x_given_h_hybrid(best_idx, use_local_search=False)
        else:
            raise ValueError(f"未知策略: {strategy}")

        print(f"改进版GP方法总共调用了 {self.evaluate_count} 次 acquisition function evaluate")
        return best_acq


